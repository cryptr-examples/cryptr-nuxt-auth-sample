...
const INIT_PROMISE = new Promise<HTTPResponse | void>((value) => value)

export class RefreshController {
  public $auth: Auth
  private _refreshPromise: Promise<HTTPResponse | void> = INIT_PROMISE
  ...
  private _doRefresh(): Promise<HTTPResponse | void> {
    this._refreshPromise = new Promise((resolve, reject) => {
      this.scheme
        .refreshTokens()
        .then((response) => {
          this._refreshPromise = INIT_PROMISE
          resolve(response)
        })
        .catch((error) => {
          this._refreshPromise = INIT_PROMISE
